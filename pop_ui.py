...
colours = {0: SILVER, 1: GREEN, 2: ORANGE, 3: BLUE, 4: RED}
gameQuit = False
...
def pygame_hex():
    ...
    for x,y in hex_centres():
        ...
        pygame.draw.polygon(screen, BLACK, list(hex_points(x,y)), 5)
        label = myfont.render(str(((index / ROWS), (index % COLUMNS))), 1, BLACK)
...
def draw_canvas(board):
    ...
    for x, y in hex_centres():
        ...
        pygame.draw.polygon(gameDisplay, BLACK, list(hex_points(x,y)), 5)
        label = myfont.render(str(board [0][index / ROWS][index % COLUMNS]['health']), 1, WHITE)
        gameDisplay.blit(label, (x-10, y-10))
        index += 1
    msg_tothlh = "Total Health = %d" % tothealth(board[0])
    pygame.draw.polygon(gameDisplay, BLACK, [(BOARD_WIDTH + X_OFFSET_1, Y_OFFSET_1), (BOARD_WIDTH + X_OFFSET_2, Y_OFFSET_1), (BOARD_WIDTH + X_OFFSET_2, Y_OFFSET_2), (BOARD_WIDTH + X_OFFSET_1, Y_OFFSET_2)])

    label = myfont.render(msg_tothlh, 1, WHITE)
    ...
    while not (gameQuit or flposGiven):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                print "Quiting Game Now! Sayonara!"
                pygame.quit()
                quit()
...
def draw_legend():
    ...
    for index in range(5):
         ...
         label = myfont.render(idxrev[index], 1, WHITE)
